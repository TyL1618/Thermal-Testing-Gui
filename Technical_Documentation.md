# Re_HV3000 — 技術文件 / Technical Documentation
...
Re_HV3000 是一套針對 **GOTECH HV3000** 熱變形 / 維卡軟化點試驗機（HDT / VICAT）所開發的桌面監控與報告軟體，以 Python + PyQt6 實作，具備即時數據監控、多通道折線圖、測試方法管理、自動報告匯出等功能。
...
Re_HV3000 is a desktop monitoring and reporting application for the **GOTECH HV3000** Heat Deflection Temperature (HDT) / Vicat Softening Point tester. Built with Python and PyQt6, it provides real-time data monitoring, multi-channel waveform display, test method management, and automated Excel report export.
...
| 網路 Network | 與機台同一 LAN，機台 IP 與 Port 請依實際環境設定 |
...
New-Item gui\__init__.py  -ItemType File
```
...
```
main.py
  └── GotechMachine (core/machine.py)
        │  signals: data_updated, status_updated, connected, reconnecting
        │
  └── MainWindow (gui/main_window.py)
        ...
        ├── ReportPanel
        └── ReconnectOverlay ←─ reconnecting signal   ──→ MainWindow._on_reconnecting()
```
...
**中文：** 負責與 GOTECH HV3000 建立 TCP 連線、發送/接收 528-byte 封包、解析 LVDT 與溫度資料，並透過 Qt 信號向 GUI 推送更新。

**English:** Handles TCP connection to the GOTECH HV3000, sends/receives 528-byte packets, parses LVDT and temperature data, and pushes updates to the GUI via Qt signals.
...
> 帳號 Username：`demo`　密碼 Password：`123456`　顯示名稱 Display Name：`開發者模式`
...
| `reconnecting` | `int` | 斷線倒數秒數（供 UI 顯示 Overlay）/ Countdown seconds before reconnect attempt |
...
| `test_method` | `str` | 預設測試方法名稱（預設 `"HDT-ASTM"`）/ Default test method name |
| `weight_g` | `float` | 預留：試片重量（g），目前未使用 / Reserved: specimen weight in g, currently unused |
...
LVDT_AD_TO_MM   = +0.000867  (最新校正值 / latest calibrated value)
```
...
- 換算係數目前為 `LVDT_AD_TO_MM = +0.000867`（正值，已更新）。早期初稿值 `-0.0001455` 為單點校正估算，偏差較大，已廢棄。  
  The conversion constant is now `LVDT_AD_TO_MM = +0.000867` (positive, updated). The earlier estimate of `-0.0001455` was a rough single-point calibration and has been superseded.  
  正式使用前請再執行 `machine.calibrate()` 確認係數是否仍需調整。  
  Before production use, run `machine.calibrate()` to verify the coefficient is still accurate.
...
- 第 5.1 節：開發者帳號更正為 `demo` / `123456`（顯示名稱：開發者模式），移除舊有帳號記載。  
  Section 5.1: Developer account corrected to `demo` / `123456` (display name: 開發者模式); removed outdated account entry.

- 第 7 節 / 第 8.3 節：`LVDT_AD_TO_MM` 由舊值 `-0.0001455` 更新為最新校正值 `+0.000867`，與 `machine.py` 實際常數同步。舊值備註保留於 8.3 節供追溯。  
  Sections 7 & 8.3: `LVDT_AD_TO_MM` updated from deprecated `-0.0001455` to latest calibrated value `+0.000867`, now in sync with `machine.py`. Legacy value noted in §8.3 for traceability.

- 第 6.1 節：補充 `reconnecting(int)` 信號說明（斷線倒數，觸發 `ReconnectOverlay`）。  
  Section 6.1: Added `reconnecting(int)` signal (countdown on disconnect, triggers `ReconnectOverlay`).

- 第 6.2 節：`ChannelData` 補充兩個遺漏欄位 `test_method`（預設 `"HDT-ASTM"`）與 `weight_g`（預留，目前未使用）。  
  Section 6.2: Added two previously undocumented `ChannelData` fields: `test_method` (default `"HDT-ASTM"`) and `weight_g` (reserved, currently unused).

- 第 3 節模組關係圖：補充 `reconnecting` 信號路徑（`GotechMachine` → `MainWindow._on_reconnecting()` → `ReconnectOverlay`）。  
  Section 3 module diagram: Added `reconnecting` signal path to the relationship diagram.

- 第 2 節安裝步驟：補充 Windows PowerShell 建立 `__init__.py` 的指令（`New-Item`），原 `touch` 指令僅適用 macOS / Linux。  
  Section 2: Added Windows PowerShell equivalent (`New-Item`) for creating `__init__.py` files; `touch` only works on macOS/Linux.
